...
function parseBuildPlans(source: string) {
  ...
  const roomBlockPattern =
    /([WE]\d+[NS]\d+):\s*{\s*plan:\s*\[([\s\S]*?)\]\s*,?\s*}/g;
...
function serializeBuildPlans(plans: unknown): string {
  const header =
  ...
  let code = JSON.stringify(plans, null, 2);
  ...
  for (const structureType of structureTypes) {
    code = code.replaceAll(`"${structureType}"`, structureType);
  }
